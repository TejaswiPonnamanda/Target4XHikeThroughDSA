...
class Solution {
    public int countPaths(int n, int[][] roads) {
        ...
        pq.add(new Pair(0,0));//dis,src
        dis[0]=0;
        ways[0]=1;
        int mod=(int)(1e9+7);
        while(!pq.isEmpty())
        {
            long curDis=pq.peek().dis;
            ...
            for(Pair it:adj.get(node))
            {
                ...
                if(curDis+edgeWeight<dis[adjNode])
                {
                    dis[adjNode]=curDis+edgeWeight;
                    ways[adjNode]=ways[node];
                    pq.add(new Pair(curDis+edgeWeight,adjNode));

                }
                else if(curDis+edgeWeight==dis[adjNode])
                {
